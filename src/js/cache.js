...
import { ONE_MONTH } from './date';

const DEFAULT_DIR_NAME = 'cache';
...
export function readCache(name, lifetime = DEFAULT_LIFETIME, extension = DEFAULT_EXTENSION) {
  if (!name) {
    return null;
  }
  if (!lifetime) {
...
export function readRawCache(name, extension = DEFAULT_EXTENSION) {
  if (!name) {
    return null;
  }
  const path = ensureFilepath(baseDir(), name, extension);
...
export async function getFromCacheOrCallback(name, callback, pauseAfter = 0, lifetime = DEFAULT_LIFETIME) {
  const fromCache = readCache(name, lifetime);
  if (fromCache?.value) {
...
export function writeCache(name, data, extension = DEFAULT_EXTENSION) {
  let str, obj;
  try {
    obj = {
      modified: Date.now(),
...
export function writeRawCache(name, data, extension = DEFAULT_EXTENSION) {
  let str, obj;
  try {
...
export function ensureFilepath(dir, filename) {
